...
for pred_tool in tqdm(pred_tools):
    if pred_tool in snakemake.input.keys():
        ...
        if pred_tool in tissue_specific_tools:
            ...
            assert len(set(df.reset_index().set_index([*index, f'tissue_{pred_tool}']).index)) == df.shape[0]
        else:
...
print(f'df_benchmark.shape[0]: {df_benchmark.shape[0]}')
print(f'universe.shape[0]: {universe.shape[0]}')
print(df_benchmark.head())
print(universe.head())
assert df_benchmark.shape[0] == universe.shape[0]
assert len(set(universe.index)) == universe.shape[0]
df_benchmark.to_parquet(snakemake.output['combined_benchmark'], index=False, partition_cols=['tissue'])
