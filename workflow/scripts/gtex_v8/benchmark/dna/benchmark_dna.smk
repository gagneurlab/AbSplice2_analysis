...
rule benchmark_combine_dna_GTEx:
    input:
    ...
    output:
        combined_benchmark = directory(OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['combined_benchmark']['dna']),
    script:
...
rule performance_dna_all_tissues:
    input:
        benchmark = expand(OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['combined_benchmark']['dna'],
    ...
    output:
        df_performance = OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['performance']['dna']['all_tissues']['df'],
        aps_performance = OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['performance']['dna']['all_tissues']['aps']
    script:
...
rule performance_dna_single_tissue:
    ...
    params:
        ...
        gene_tpm_cutoff = config['filtering_params']['absplice']['tpm_cutoff'],
    output:
        df_performance = OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['performance']['dna']['single_tissue']['df'],
        aps_performance = OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['performance']['dna']['single_tissue']['aps']
    script:
        "../../../../scripts/common/benchmark/performance_dna.py"
...
rule performance_dna_across_tissues_boxplot:
    input:
        benchmark = expand(OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['performance']['dna']['single_tissue']['aps'],
                           tissue=config['tissues'], classifier='{classifier}',
    ...
    resources:
        mem_mb = lambda wildcards, attempt: attempt * 64000,
    output:
        df_performance_boxplot = OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['performance']['dna']['across_tissues']['boxplot_aps'],
    script:
...
rule corresponding_thresholds:
    ...
    resources:
        mem_mb = lambda wildcards, attempt: attempt * 64000,
    output:
        thresholds = OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['performance']['dna']['thresholds']['corresponding_thresholds'],
        thresholds_per_model = OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['performance']['dna']['thresholds']['thresholds_per_model'],
    script:
...
rule threshold_points_pr_curve:
    ...
    resources:
        mem_mb = lambda wildcards, attempt: attempt * 64000,
    output:
        threshold_points_pr_curve = OUTPUT_DIR_BENCHMARK + config['benchmark']['absplice_model_params'] + config['benchmark']['performance']['dna']['thresholds']['threshold_points_pr_curve'],
    script:
...
rule all_benchmark_dna:
    input:
        filter_wildcards_outliers(expand(rules.performance_dna_all_tissues.output,
        ...
        filter_wildcards_outliers(expand(rules.performance_dna_across_tissues_boxplot.output,
               classifier=config['absplice_params']['classifier'],
               abs_features=config['absplice_params']['abs_features'],
               feature_string_dna=config['absplice_params']['feature_string_dna'],
               fraser_version=config['fraser_version'], 
        ...
        filter_wildcards_outliers(expand(rules.threshold_points_pr_curve.output,
               classifier=config['absplice_params']['classifier'],
               abs_features=config['absplice_params']['abs_features'],
               feature_string_dna=config['absplice_params']['feature_string_dna'],
               fraser_version=config['fraser_version'], 
