...
# read in dev and GTEx predictions            
df = pd.read_parquet(snakemake.input['combined'])
# df = df[
#     (df['max_all'] >= float(snakemake.params['high_cutoff']))
#     & (df['max_all_GTEx'] <= float(snakemake.params['low_cutoff']))
# ]

# Define the tissues you want to compare
tissues = ["brain", "cerebellum", "heart", "kidney", "liver", "ovary", "testis"]

# Create condition for each tissue
conditions = [
    (df[f"{t}_max_early"] > 0.2) & (df[f"{t}_max_GTEx"] < 0.05)
    for t in tissues
]

# Combine all tissue conditions with OR
...
#     df['variant'] = df.apply(lambda x: f"{x['chrom']}:{x['end']}:{x['ref']}>{x['alt']}", axis=1)
# variants = sorted(set(df['variant']))
# variants = [Variant.from_str(x) for x in variants]

# # save to vcf
# to_vcf(
#     variants, 
#     snakemake.output['vcf'], 
#     snakemake.params['chr_annotation'], 
#     snakemake.params['genome_version']
# )

df.to_parquet(snakemake.output['variants'], index=False, partition_cols=['chrom'])