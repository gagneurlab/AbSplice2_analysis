...
# # get coords for SpliceMap splice sites
_df_splicemap1 = df_splicemap.copy()
_df_splicemap2 = df_splicemap.copy()

_df_splicemap1['Chromosome'], _df_splicemap1['Start'], _df_splicemap1['End'] = zip(*_df_splicemap1['junctions'].apply(
    lambda x: get_pr_coords_junctions(x, 'j1')
))
_df_splicemap2['Chromosome'], _df_splicemap2['Start'], _df_splicemap2['End'] = zip(*_df_splicemap2['junctions'].apply(
...
if 'Pangolin_max_score' not in df_pangolin.columns:
    df_pangolin['Pangolin_max_score'] = df_pangolin[['gain_score', 'loss_score']].abs().max(axis=1)
pangolin_cols = ['variant', 'gene_id', 'gain_score', 'gain_pos', 'loss_score', 'loss_pos', 'Pangolin_max_score']
...
input_index = [
    ...
    'gene_id',
    'tissue',
    'gain_score', 'gain_pos', 'loss_score', 'loss_pos', 'Pangolin_max_score'
    # 'junctions',
    # 'event_type',
]
...
print(f'df.shape: {df.shape}')
print(f"df[['variant', 'gene_id', 'tissue']].drop_duplicates().shape: {df[['variant', 'gene_id', 'tissue']].drop_duplicates().shape}")
print(f'df_variants_tissues.shape: {df_variants_tissues.shape}')
print(f"df_variants_tissues[['variant', 'gene_id', 'tissue']].drop_duplicates().shape: {df_variants_tissues[['variant', 'gene_id', 'tissue']].drop_duplicates().shape}")
...
# seq_depth_factor = float(snakemake.params['seq_depth_factor'])
seq_depth_factor = 1
df['median_n_pangolin_adj_seq_depth'] = df['median_n_pangolin'] * seq_depth_factor
...
if 'mmsplice_splicemap' in snakemake.input.keys():
    ...
    print(f'df_mmsplice_splicemap.shape: {df_mmsplice_splicemap.shape}')
    if 'variant' in df_mmsplice_splicemap.columns:
        df_mmsplice_splicemap = df_mmsplice_splicemap.rename(columns={
            'junction': 'junctions_mmsplice',
        })
        print('Before MMsplice join: df.shape:', df.shape)
        print('Before MMsplice join: df_mmsplice_splicemap.shape:', df_mmsplice_splicemap.shape)
        df = df.set_index(['variant', 'gene_id', 'tissue']).join(
            df_mmsplice_splicemap.set_index(['variant', 'gene_id', 'tissue'])[[
                'junctions_mmsplice', 'delta_logit_psi', 'delta_psi', 'median_n', 'ref_psi'
            ]], how='outer'
        ).reset_index()
        print('After MMsplice join: df.shape:', df.shape)
    else:
        df_mmsplice_splicemap['variant'] = df_mmsplice_splicemap.apply(
            lambda row: f'{row["chrom"]}:{row["pos"]}:{row["ref"]}>{row["alt"]}', axis=1)
        df = df.set_index(['variant', 'gene_id', 'tissue']).join(
            df_mmsplice_splicemap.set_index(['variant', 'gene_id', 'tissue'])[[
                'junction_start', 'junction_end', 'junction_strand', 'delta_logit_psi', 'delta_psi', 'median_n', 'ref_psi'
            ]], how='outer'
        ).reset_index()
...
for model_name, model_info in absplice_models.items():
    ...
    print(f'len(absplice_model_names): {len(absplice_model_names)}')
    df = optimize_dataframe(df)
