...
if len(snakemake.input['gene_tpm']) == 0:
    gene_tpm = None
else:
    gene_tpm = pd.read_csv(snakemake.input['gene_tpm'])
...
if df_mmsplice.shape[0] == 0 or df_spliceai.shape[0] == 0:
    if df_mmsplice.shape[0] == 0:
        ...
        df5 = pd.concat(
            [SpliceMap.read_csv(i).df.assign(tissue=t) for i,t in tqdm(zip(snakemake.input['splicemap_5'], snakemake.params['tissues']))]
        )
        df3 = pd.concat(
            [SpliceMap.read_csv(i).df.assign(tissue=t) for i,t in tqdm(zip(snakemake.input['splicemap_3'], snakemake.params['tissues']))]
        )
