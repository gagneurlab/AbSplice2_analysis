...
if df_mmsplice_splicemap.empty:
    required_cols = ['chrom', 'start', 'end', 'ref', 'alt', 'gene_id', 'tissue', 'junction', 'delta_logit_psi', 'delta_psi', 'median_n', 'ref_psi']
...
df['gain_score_original'] = df['gain_score'].copy()
if 'clip_pangolin' in snakemake.params.keys() and snakemake.params['clip_pangolin']:
    df['gain_score'] = df['gain_score'].clip(upper=0.7)
elif 'clip_pangolin' not in snakemake.params.keys():
    df['gain_score'] = df['gain_score'].clip(upper=0.7)
else:
    pass
...
df['gain_score'] = df['gain_score_original'].copy()
