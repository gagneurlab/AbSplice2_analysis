...
rule variant_level_squirls:
    ...
    output:
        model_agg = directory(OUTPUT_DIR_SPLICING_AGG + config_static['splicing_pred']['models']['squirls'])
    script:
        "./max_aggregation.py"
...
rule variant_level_spliceai:
    ...
    params:
        ...
        groupby_index = groupby_index,
    output:
        model_agg = directory(OUTPUT_DIR_SPLICING_AGG + config_static['splicing_pred']['models']['spliceai'])
    script:
        "./max_aggregation.py"
...
rule variant_level_pangolin:
    ...
    resources:
        ...
        threads = 1
    params:
        max_col = 'Pangolin_max_score',
        groupby_index = groupby_index,
    output:
        model_agg = directory(OUTPUT_DIR_SPLICING_AGG + config_static['splicing_pred']['models']['pangolin'])
    script:
        "./max_aggregation.py"
...
rule variant_level_mmsplice:
    ...
    params:
        ...
        groupby_index = groupby_index,
    output:
        model_agg = directory(OUTPUT_DIR_SPLICING_AGG + config_static['splicing_pred']['models']['mmsplice'])
    script:
        "./max_aggregation.py"
...
rule variant_level_cadd_splice:
    ...
    params:
        ...
        groupby_index = groupby_index,
    output:
        model_agg = directory(OUTPUT_DIR_SPLICING_AGG + config_static['splicing_pred']['models']['cadd_splice'])
    script:
        "./max_aggregation.py"
...
# AbSplice-DNA custom old
if 'absplice_dna_custom_old' in config['models']:
    rule variant_level_absplice_dna_custom_old:
        input:
            model = OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['absplice_custom_old']['dna'],
        resources:
            mem_mb = lambda wildcards, attempt: attempt * 64000,
        params:
            absplice_models = config["absplice_models_old"],
            groupby_index = groupby_index_tissue,
        output:
            model_agg = directory(OUTPUT_DIR_SPLICING_AGG + config_static['splicing_pred']['models']['absplice_custom_old']['dna'])
        script:
            "./max_aggregation_several_models.py"

# AbSplice-DNA custom new
if 'absplice_dna_custom_new' in config['models']:
    rule variant_level_absplice_dna_custom_new:
        input:
            model = OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['absplice_custom_new']['dna'],
        resources:
            mem_mb = lambda wildcards, attempt: attempt * 64000,
        params:
            ...
            groupby_index = groupby_index_tissue,
        output:
            model_agg = directory(OUTPUT_DIR_SPLICING_AGG + config_static['splicing_pred']['models']['absplice_custom_new']['dna'])
        script:
            "./max_aggregation_several_models.py"
...
rule variant_level_absplice_dna_extra_info:
    ...
    resources:
        mem_mb = lambda wildcards, attempt: attempt * 64000,
    output:
        absplice_extra_info = directory(OUTPUT_DIR_SPLICING_AGG + config_static['splicing_pred']['models']['absplice_extra_info']['dna'])
    script:
...
rule variant_level_absplice_dna_no_samples_extra_info:
    ...
    resources:
        mem_mb = lambda wildcards, attempt: attempt * 64000,
    output:
        absplice_extra_info = directory(OUTPUT_DIR_SPLICING_AGG + config_static['splicing_pred']['models']['absplice_extra_info']['dna_no_samples'])
    script:
        "./absplice_dna_add_extra_info.py"
...
if 'pangolin' in config['models']:
    list_outputs.append(
        expand(rules.variant_level_pangolin.output, 
        vcf_id=wildcard_vcf_id),
    )
...
if 'absplice_dna_custom_old' in config['models']:
    list_outputs.append(
        expand(rules.variant_level_absplice_dna_custom_old.output, 
        vcf_id=wildcard_vcf_id), # AbSplice-DNA
    )
if 'absplice_dna_custom_new' in config['models']:
    list_outputs.append(
        expand(rules.variant_level_absplice_dna_custom_new.output, 
        vcf_id=wildcard_vcf_id), # AbSplice-DNA
    )
