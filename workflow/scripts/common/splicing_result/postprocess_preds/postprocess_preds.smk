...
rule splicing_result_postprocess_mmsplice:
    ...
    resources:
        ...
        threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['mmsplice'])
    script:
...
rule splicing_result_postprocess_mtsplice:
    ...
    resources:
        ...
        threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['mtsplice'])
    script:
        "./mtsplice.py"
...
rule splicing_result_postprocess_squirls:
    input:
        ...
        var_samples_df = var_samples_df,
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['squirls'])
    script:
        "./squirls.py"
...
rule splicing_result_postprocess_pangolin:
    input:
        model = OUTPUT_DIR_SPLICING_RAW + config_static['splicing_pred']['models']['pangolin_raw'],
        var_samples_df = var_samples_df,
    resources:
        mem_mb = lambda wildcards, attempt: attempt * 64000,
        threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['pangolin'])
    script:
        "./pangolin.py"

# Pangolin
rule splicing_result_postprocess_pangolin_in_raw:
    input:
        model = OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['pangolin'],
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_RAW + config_static['splicing_pred']['models']['pangolin']),
    shell:
        '''
        if [ -d {input} ]; then
            cp -r {input} {output}
...
rule splicing_result_postprocess_spliceai:
    ...
    resources:
        ...
        threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['spliceai'])
    script:
        "./spliceai.py"
...
rule splicing_result_postprocess_spliceai_splicemap:
    ...
    resources:
        ...
        threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['spliceai_splicemap']['all'])
    script:
...
rule splicing_result_postprocess_spliceai_splicemap_ref_psi:
    ...
    resources:
        ...
        threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['spliceai_splicemap_ref_psi']['all'])
    script:
        "./spliceai_splicemap.py"
...
rule splicing_result_postprocess_cadd_splice:
    ...
    resources:
        ...
        threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['cadd_splice'])
    script:
        "./cadd_splice.py"
...
rule splicing_result_postprocess_mmsplice_splicemap:
    ...
    resources:
        ...
        threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['mmsplice_splicemap'])
    script:
...
rule splicing_result_postprocess_mmsplice_splicemap_ref_psi:
    ...
    resources:
        ...
        threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['mmsplice_splicemap_ref_psi'])
    script:
        "./mmsplice_splicemap.py"
...
rule splicing_result_postprocess_cat_infer_single_cat:
    ...
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['mmsplice_splicemap_cat']['single_cat']),
    shell:
        '''
        if [ -d {input} ]; then
            cp -r {input} {output}
...
rule splicing_result_postprocess_cat_infer_all_cats:
    ...
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['mmsplice_splicemap_cat']['all_cats']),
    shell:
        '''
        if [ -d {input} ]; then
            cp -r {input} {output}
...
rule splicing_result_postprocess_absplice_dna:
    ...
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['absplice']['dna']),
    shell:
        '''
        if [ -d {input} ]; then
            cp -r {input} {output}
...
# AbSplice-DNA
rule splicing_result_postprocess_absplice_dna_custom_old:
    input:
        model = OUTPUT_DIR_SPLICING_RAW + config_static['splicing_pred']['models']['absplice_custom_old']['dna'],
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['absplice_custom_old']['dna']),
    shell:
        '''
        if [ -d {input} ]; then
            cp -r {input} {output}
...
# AbSplice-DNA
rule splicing_result_postprocess_absplice_dna_custom_new:
    input:
        model = OUTPUT_DIR_SPLICING_RAW + config_static['splicing_pred']['models']['absplice_custom_new']['dna'],
        var_samples_df = var_samples_df,
    resources:
            mem_mb = lambda wildcards, attempt: attempt * 128000,
            threads = 1
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['absplice_custom_new']['dna']),
    script:
        "./absplice_dna_custom_new.py"
...
rule splicing_result_postprocess_absplice_dna_no_samples:
    ...
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['absplice']['dna_no_samples']),
    shell:
        '''
        if [ -d {input} ]; then
            cp -r {input} {output}
...
rule splicing_result_postprocess_absplice_rna_single_cat:
    ...
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['absplice']['rna']['single_cat']),
    shell:
        '''
        if [ -d {input} ]; then
            cp -r {input} {output}
...
rule splicing_result_postprocess_absplice_rna_all_cats:
    ...
    output:
        model_postprocess = directory(OUTPUT_DIR_SPLICING_POSTPROCESS + config_static['splicing_pred']['models']['absplice']['rna']['all_cats']),
    shell:
        '''
        if [ -d {input} ]; then
            cp -r {input} {output}
...
if 'pangolin' in config['models']:
    list_outputs.append(
        expand(rules.splicing_result_postprocess_pangolin.output, 
        vcf_id=wildcard_vcf_id), # MMSplice + SpliceMap
    )
if 'pangolin' in config['models']:
    list_outputs.append(
        expand(rules.splicing_result_postprocess_pangolin_in_raw.output, 
        vcf_id=wildcard_vcf_id), # MMSplice + SpliceMap
    )
