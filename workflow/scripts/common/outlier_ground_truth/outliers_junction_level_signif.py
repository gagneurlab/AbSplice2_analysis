...
PADJUST_CUTOFF = float(snakemake.wildcards['padjust_cutoff'])
TOTAL_COUNTS_CUTOFF = float(snakemake.wildcards['totalCounts_cutoff'])
...
# filter by cutoffs
df_junction_level = df_junction_level[
    (df_junction_level['padjust'] <= PADJUST_CUTOFF)
    & (df_junction_level['totalCounts'] >= TOTAL_COUNTS_CUTOFF)
...
if snakemake.wildcards['fraser_version'] == 'fraser1':
...
elif snakemake.wildcards['fraser_version'] == 'fraser2':
    df_junction_psi5 = df_junction_level[df_junction_level['type'] == 'jaccard'].set_index('junctions')
    df_junction_psi3 = df_junction_level[df_junction_level['type'] == 'jaccard'].set_index('junctions')
    ...
    pd.concat([
        df_junction_psi5, 
        df_junction_psi3,
    ]).drop_duplicates().to_csv(snakemake.output['junction_signif'])